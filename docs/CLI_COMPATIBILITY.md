...
- **既有 Antigravity 連接**：卡片設定會保留，但正式版不執行 `1.1.7`／`1.1.9` 的舊流程；仍使用這些 CLI 版本時無法取得新用量。請依[使用說明](../使用說明.md#支援平台)更新 CLI，再重新連接。
...
本表只對應 `1.0.13 / sequence 1027`（[source `f98b36cc9efd78b6ae2a7cf38956241f1690aef8`](https://github.com/Sokaka/ai-usage-dashboard/commit/f98b36cc9efd78b6ae2a7cf38956241f1690aef8)）的同一組正式公開成品。本版沿用前一正式版已簽署的 `1.0.12` Updater 原始 bytes；上列 CLI 版本要求仍適用。
...
[同 source Windows CI](https://github.com/Sokaka/ai-usage-dashboard/actions/runs/36232019499) 已通過 Release 建置（0 warnings／0 errors）、完整測試（4,320 passed／0 failed／0 skipped）與 production line coverage 74.27%（48,540／65,360）。[候選 workflow](https://github.com/Sokaka/ai-usage-dashboard/actions/runs/36232360073/attempts/1) attempt 1 亦通過相同測試數與覆蓋率，並簽署凍結六件成品；公開後匿名下載六件原成品，sidecar 與 latest stable feed 的 production public trust 簽章驗證通過。這些結果不代替乾淨 Windows 初裝、真人 CLI 登入與用量查詢、舊 Updater 交接或 App 一鍵線上更新；先前版本的實測不沿用為本版結果。成品與發布驗證範圍見 [1.0.13 版本說明](releases/1.0.13.md)。

### 1.0.12

本表只對應 `1.0.12 / sequence 1026`（[source `1f7dfa10c606e500096f7af3472ee3277a94615a`](https://github.com/Sokaka/ai-usage-dashboard/commit/1f7dfa10c606e500096f7af3472ee3277a94615a)）的同一組正式公開成品。本版建置新的 `1.0.12` Updater；上列 CLI 版本要求仍適用。
...
[同 source Windows CI](https://github.com/Sokaka/ai-usage-dashboard/actions/runs/36222460207) 與[候選 workflow](https://github.com/Sokaka/ai-usage-dashboard/actions/runs/36222763387/attempts/1) 均通過。候選 Release 建置 0 warnings／0 errors、完整自動測試 4,320 passed／0 failed／0 skipped，production line coverage 74.27%（48,540／65,360）；同一個 attempt 簽署、驗證並凍結六件成品。正式公開後已匿名下載六件原凍結成品，核對 Release 與 asset 身分、大小、SHA256、三份 sidecar、stable feed 簽章及 latest feed bytes。這些結果不代替乾淨 Windows 的首次安裝條款接受／拒絕、真實 CLI 登入與用量查詢或 App 一鍵線上更新；先前版本的實測不沿用為本版結果。成品與發布驗證範圍見 [1.0.12 版本說明](releases/1.0.12.md)。

### 1.0.11

本表只對應 `1.0.11 / sequence 1025`（[source `ce5ad8512a364b676e169eb61de508bd90d5b747`](https://github.com/Sokaka/ai-usage-dashboard/commit/ce5ad8512a364b676e169eb61de508bd90d5b747)）的同一組正式公開成品。本版沿用已簽署的 `1.0.8` Updater 原始 bytes；上列 CLI 版本要求仍適用。
...
[同 source Windows CI](https://github.com/Sokaka/ai-usage-dashboard/actions/runs/36217678379) 與[候選 workflow](https://github.com/Sokaka/ai-usage-dashboard/actions/runs/36218285497) attempt 1 已通過。候選 Release 建置 0 warnings／0 errors、完整自動測試 4,320 passed／0 failed／0 skipped，production line coverage 74.26%（48,540／65,361）；同一個 attempt 簽署、驗證並凍結六件成品。正式公開後已匿名下載六件原凍結成品，核對 Release 與 asset 身分、大小、SHA256、三份 sidecar、stable feed 簽章及 latest feed bytes。這些結果不代替實際 CLI 登入、用量查詢、安裝或 App 按鈕的 signed online update；先前版本的實測不沿用為本版結果。成品與發布驗證範圍見 [1.0.11 版本說明](releases/1.0.11.md)。
...
[候選 workflow](https://github.com/Sokaka/ai-usage-dashboard/actions/runs/35752107845) attempt 2 的 Release 建置為 0 warnings／0 errors、完整自動測試 4,307 項通過，production line coverage 75.09%；同一個 attempt 簽署、驗證並凍結六件成品。2026-09-23 已將 exact candidate 以 `apply-local` 套用至既有標準安裝，核對完整 payload、維護 Updater、交易、登錄與捷徑，並完成四個停靠角的展開首幀取樣及四個一般主題的收合按鈕／捲動條檢查。正式公開後另以匿名 client 下載六件原凍結成品，核對 tag target、asset identity、大小、SHA256、sidecar 與 stable feed 簽章。這些結果不代替 signed online update、乾淨 Windows 安裝、High Contrast／多螢幕／完整無障礙或真人 provider 驗收。成品與發布驗證範圍見 [1.0.6 版本說明](releases/1.0.6.md)。
